  ...
  return new Promise((resolve) => {
    let options = {
      ...
      "headers":  {
        ...
        "User-Agent": $.isNode() ? (process.env.JD_USER_AGENT ? process.env.JD_USER_AGENT : (require('./USER_AGENTS').USER_AGENT)) : ($.getdata('JDUA') ? $.getdata('JDUA') : "jdapp;iPhone;9.4.4;14.3;network/4g;Mozilla/5.0 (iPhone; CPU iPhone OS 14_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148;supportJDSHWK/1"),
        'openId' : `oPcgJ4_HwICEIlB1eqwTUzEYcWE0`,
...
async function invite() {
  const url = `https://draw.jdfcloud.com/common/api/bean/activity/invite?openId=oPcgJ4_HwICEIlB1eqwTUzEYcWE0&activityId=${$.activityId}&userSource=mp&formId=123&jdChannelId=&fp=&appId=wxccb5c536b0ecd1bf&invokeKey=NRp8OPxZMFXmGkaE`;
  const method = `POST`;
  const headers = {
    'content-type' : `application/json`,
    'Connection' : `keep-alive`,
...
async function getActivityDetail() {
  const url = `https://draw.jdfcloud.com/common/api/bean/activity/detail?activityId=${$.activityId}&userOpenId=oPcgJ4_HwICEIlB1eqwTUzEYcWE0&timestap=${Date.now()}&userSource=mp&jdChannelId=&appId=wxccb5c536b0ecd1bf&invokeKey=NRp8OPxZMFXmGkaE`;
  const method = `GET`;
  const headers = {
    'cookie' : $.cookie,
    'openId' : `oPcgJ4_HwICEIlB1eqwTUzEYcWE0`,
