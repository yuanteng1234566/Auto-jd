...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
   ...
   let res = [];
   try{res = await getAuthorShareCode('https://raw.githubusercontent.com/asd920/Auto-jd/main/backUp/ProductZ4Brand.json');}catch (e) {}
   if(!res){
     try{res = await getAuthorShareCode('https://gitee.com/asd920/Auto-jd/raw/main/backUp/ProductZ4Brand.json');}catch (e) {}
     if(!res){res = [];}
   }
   for (let i = 0; i < 1; i++) {
     $.cookie = cookiesArr[i];
